...
### 🟡 개선 사항 (Phase 2에서 해결 예정)

1. **스냅샷 후 오버레이 잔류** — 스냅샷 번역 후 결과가 화면에 계속 남음 → P2-2
2. **단일 OCR 영역 제한** — 1개 영역만 지정 가능 → P2-4
3. **좌표 기반 캡처만 지원** — 창 이동 시 영역 어긋남 → P2-6
4. **캐시 비영속** — 앱 재시작 시 번역 캐시 소멸 → P2-1
...
> **반드시 `docs/Phase2-Spec.md`를 먼저 읽고 시작할 것**
> 각 기능의 상세 구현 스펙, 수정 대상 파일, 엣지 케이스, 테스트 체크리스트가 모두 기록되어 있음

### 구현 순서

| 순서 | ID | 기능 | 난이도 | 예상 시간 | 핵심 수정 파일 |
|------|----|------|--------|-----------|----------------|
| 1 | P2-1 | 영속 캐시 (디스크 저장) | 하 | 1~2시간 | `cache.py`, `main.py` |
| 2 | P2-2 | 스냅샷 오버레이 정리 | 하 | 1시간 | `overlay.py`, `hotkeys.py`, `main.py` |
| 3 | P2-3 | OCR 전처리 미리보기 | 하 | 2~3시간 | `ocr.py`, `ui/ocr_preview.py`(신규), `tray.py` |
| 4 | P2-4 | 다중 OCR 영역 | 중상 | 6~8시간 | `config.py`, `capture.py`, `detector.py`, `pipeline.py`, `overlay.py`, `region_selector.py`, `main.py` |
| 5 | P2-5 | 영역 크기 조정·삭제 UI | 중 | 3~4시간 | `ui/region_editor.py`(신규), `area_indicator.py`, `tray.py`, `main.py` |
| 6 | P2-6 | 윈도우 지정 캡처 + 제외 영역 | 중 | 4~5시간 | `capture.py`, `config.py`, `settings.py`, `region_editor.py` |
| 7 | P2-7 | 클립보드 자동 복사 | 하 | 30분 | `config.py`, `settings.py`, `main.py` |

### 각 작업 완료 후 할 것

1. 구현 → 테스트 (Phase2-Spec.md의 테스트 체크리스트 활용)
2. SESSION_HANDOFF.md 작업 이력 업데이트
3. ROADMAP.md 상태 업데이트 (⬜ → ✅)
