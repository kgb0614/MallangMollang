...
> 최종 업데이트: 2026-06-15 (Phase 3 완료 + UI/UX 폴리시)
> 현재 브랜치: main
...
| `core/user_dict.py` | ✅ 완료 | 사용자 번역 사전 (JSON 영속, 완전/부분 일치) |
...
| `ui/settings.py` | ✅ 완료 | 8탭 + 사용자 사전 + OCR 전처리 + 자동 색상 + 다크 테마 |
| `ui/control_panel.py` | ✅ 완료 | 플로팅 패널 (시작/중지/영역/설정 + 빠른 설정 펼침) |
...
**Phase 1~3 기능 구현 + UI/UX 폴리시 완료.** 컨트롤 패널 빠른 설정 추가, 설정 창 다크 테마 통일까지 완료.
...
- 스타일 프리셋 커스텀 저장 UI
- 추가 LLM 프로바이더 (OpenAI, Claude, Ollama) — 배포 시
