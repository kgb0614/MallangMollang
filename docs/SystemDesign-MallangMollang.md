...
**작성일:** 2026-06-01 (최종 수정: 2026-06-15)
**상태:** 승인됨 (Phase 3 + UI/UX 폴리시 반영)
...
**Settings** — 설정 창
- 탭 구조: 프로바이더, 언어, 캡처, 번역, 표시, 번역 프로필, 사용자 사전, 단축키 (8탭)
- 각 탭이 Config 모듈을 통해 설정을 읽고 저장
- 컨트롤 패널과 통일된 다크 네이비 테마 스타일시트 적용
- 번역 프로필 탭: 키워드 자동 생성, 수동 편집 (이름/장르/분위기/용어집/추가지시), 저장/삭제
- 사용자 사전 탭: 원문→번역 쌍 테이블, 추가/삭제
- 캡처 탭: OCR 전처리 (밝기/대비/그레이스케일) 포함

**ControlPanel** — 컨트롤 패널
- 플로팅 미니 패널 (시작/중지, 영역 선택, 설정, 종료)
- 상태 표시 바 (번역 중/대기/오류)
- 드래그 이동 가능
- 펼침 영역 (▼ 더보기): 번역 모드(실시간/스냅샷), 표시 모드(오버레이/패널), 클립보드 복사, OCR 미리보기
- 설정 창과 양방향 동기화 (blockSignals 기반)
...
│   ├── settings.py           # 설정 창 (8탭: 프로바이더/언어/캡처/번역/표시/프로필/사전/단축키, 다크 테마)
│   ├── control_panel.py      # 플로팅 컨트롤 패널 (시작/중지/영역/설정 + 빠른 설정 펼침)
